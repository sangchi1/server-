var md = require('../models/laptop');
var mdTheLoai = require('../models/the_loai');

exports.list = async(req,res,next) =>{

    try {
        const laptop = await md.LapTopModel.find({});
        res.json(laptop);
      } catch (err) {
...
exports.getAll = async(req,res,next) =>{
   ...
   try{
    var listAll = await md.LapTopModel.find();
    var data = [];
  ...
  exports.getChiTiet = async(req,res,next)=>{
    ...
    try{
      ...
      var ChiTiet = await md.LapTopModel.findById(id);
      var data = [];
  ...
  exports.add = async (req, res, next) => {
      ...
      const { tensp,mota,giasp,hinhanh,soluong,the_loai} = req.body;
      const newLapTop = new md.LapTopModel({
         tensp: tensp,
          mota: mota,
          giasp: giasp,
          hinhanh:hinhanh,
